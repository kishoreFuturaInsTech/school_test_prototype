...
import QuestionTypeAdd from "./QuestionTypeAdd";
...
function OuestionType() {
  ...
  const globalsearch = (val) => {
    val === ""
      ? getData()
      : axios
          .get(`http://localhost:8090/questionType/search/${val}`, {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
          })
  ...
  return (
    <div>
      ...
      <Modal show={editOpen} onHide={editClickClose} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title> Update Question Type </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="container">
            <QuestionTypeEdit
